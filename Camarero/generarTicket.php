...
try {
     ...
     $queryPedido = "SELECT idPedido,fecha FROM pedido WHERE pagado=0 AND numMesa='$numMesa'";

     $resultPedido = mysqli_query($conn, $queryPedido);

     $rowP = mysqli_fetch_assoc($resultPedido);

     $rowP = $idPedido['idPedido'];

     $$idPedido=$rowP['idPedido'];
     $fecha=$rowP['fecha'];
     // Generar número de factura
     $num_factura = $fecha.$idPedido;
